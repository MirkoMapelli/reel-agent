...
(function voiceModule() {
  ...
  function renderVoiceGrid() {
    ...
    grid.querySelectorAll('.voice-thumb-video').forEach(v => {
      ...
      v.addEventListener('loadedmetadata', () => {
        v.currentTime = 0.01; // forza il rendering del primo frame
      });
      ...
      // Hover: play se non sta registrando
      const card = v.closest('.voice-card');
      card.addEventListener('mouseenter', () => {
        if (!v.dataset.recording) {
          v.currentTime = 0;
          v.play().catch(() => {});
        }
      });
  ...
  async function toggleRecord(sid, btn) {
    ...
    try {
      ...
      // Avvia il video della scena e loop durante la registrazione
