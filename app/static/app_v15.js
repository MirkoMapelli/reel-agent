      ...
      return `
        <div class="voice-card ${has ? 'recorded' : ''}" data-sid="${sid}">
          ...
          <div class="voice-actions-cell">
            ...
            <button class="btn-mini btn-rec-del" data-sid="${sid}" ${has ? '' : 'disabled'} title="Cancella registrazione">🗑 Cancella</button>
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          const r = await fetch('/api/voice/delete', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ edl_job_id: currentEdlJobId, scene_id: sid })
          });
          const d = await r.json();
          if (!r.ok) throw new Error(d.error || 'errore');
...
(function bindReburn() {
  ...
  btn.addEventListener('click', async () => {
    if (!currentEdlJobId) { alert('Nessun EDL caricato'); return; }
    ...
    try {
      const r = await fetch(`/api/render/${currentEdlJobId}/reburn`, { method: 'POST' });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || 'errore');
      if (typeof startJobSSE === 'function') startJobSSE(d.job_id);
    } catch (e) {
